  ...
  return (
    <div>
      ...
      <div className="flex flex-col items-center gap-3 md:flex-row md:justify-between">
        <button
          className="border cursor-pointer rounded-full py-2 border-slate-200 px-5 flex items-center justify-center gap-2 text-xs font-medium uppercase tracking-widest hover:bg-stone-100 transition-all self-start"
          onClick={() => navigate(-1)}
        >
        ...
        <div className="flex flex-col items-center gap-1.5">
          <h1 className="text-2xl md:text-3xl font-bold">Pedido #{id}</h1>
          <p className="text-sm">{formatDateLong(order.created_at)}</p>
        </div>
        <div className="hidden md:block" />
      </div>

      <div className="flex flex-col mt-6 md:mt-10 mb-5 gap-8 md:gap-10">
        
        <div className="flex flex-col gap-3 md:hidden">
          {order.orderItems.map((item, index) => (
            <div
              key={index}
              className="border border-gray-200 rounded-lg p-3 flex gap-3 items-center"
            >
              <img
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="flex flex-col gap-3">
            ...
            <div className="border border-stone-300 p-4 md:p-5 flex flex-col gap-5 rounded-lg">
              ...
              <div className="flex flex-col gap-1 text-sm">
                ...
                <p>
                  {order.address.postalCode && <>Código postal: </>}
