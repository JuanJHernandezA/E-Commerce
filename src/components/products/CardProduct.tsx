...
const CardProduct = ({ img, name, price,  slug, colors, variants }: Props) => {
  ...
  return (
    <div className="flex flex-col gap-6 relative">
      <Link to={`/celulares/${slug}`} className='group relative flex overflow-hidden'>
        <div className="flex h-[350px] w-full items-center justify-center py-2 lg:h-[250px]" >
            <img src={img} alt={name} className="object-contain object-containt h-full w-full" />
        </div>
        ...
        gap-1 text-sm font-medium hover:bg-stone-100 translate-y-[100%] transition-all duration-300 group-hover:translate-y-0' onClick={handleAddClick}>
            <FiPlus />
            Añadir
        </button>
      </Link>
