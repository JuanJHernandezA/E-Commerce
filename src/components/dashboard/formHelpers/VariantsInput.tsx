...
const headersVariants = ["Stock", "Precio", "Almacenamiento", "Color", ""];
const VariantsInput = ({ control, errors, register }: Props) => {
  ...
  const addVariant = () => {
    append({ stock: 0, price: 0, storage: "", color: "", colorName: " " });
  };
  ...
  const colorValues = useWatch({control,name:fields.map((_,index)=> `variants.${index}.color` as const)})
   const colorNameValues = useWatch({control,name:fields.map((_,index)=> `variants.${index}.colorName` as const)})

   const getFirstError =(variantsErrors: FieldErrors<ProductFormValues['variants'][number]>)=>{
    if(variantsErrors) {
  ...
  return (
    <div className="flex flex-col gap-3 ">
              ...
              <div className="flex relative">
                {colorActive[index] && (
                  <div className="absolute bg-stone-100 rounded-md bottom-8 left-[40px] p-1 w-[100px] h-fit space-y-2">
                    ...
                    <input
                      ...
                      className="rounded-md px-3 py-1.5 w-full text-xs  focus:outline-none font-semibold placeholder:font-normal bg-white placeholder:text-black"
                    />
                  </div>
                )}
                <button
                  className="border w-full h-8 cursor-pointer rounded text-xs font-medium flex items-center
                    justify-center"
                  type="button"
                ...
                >
                  {colorValues[index] && colorNameValues[index] ? (
                    <span
                      className={`inline-block w-4 h-4 rounded-full bg-block`} style={{backgroundColor:colorValues[index]}}
                    />
                  ) : (
                    "Añadir"
                  )}
      ...
      <button
       ...
       tracking-tight flex itemx-center gap-1 self-center hover:bg-slate-100 cursor-pointer"
      >
        {" "}
